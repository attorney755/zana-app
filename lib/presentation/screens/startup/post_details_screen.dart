...
class _PostDetailsScreenState extends State<PostDetailsScreen> {
    ...
    if (confirmed == true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(nextState ? 'Opportunity reopened successfully! Redirecting to My Posts...' : 'Opportunity closed successfully! Redirecting to My Posts...'),
            backgroundColor: btnColor,
  ...
  Future<void> _deletePost(OpportunityModel opp) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text('Delete Post', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1E1B4B))),
        content: Text('Are you sure you want to delete "${opp.title}"? This action cannot be undone.'),
        actions: [
          Row(
            children: [
              Expanded(
    ...
    if (confirmed == true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Opportunity deleted successfully. Redirecting to My Posts...'),
      ...
      builder: (context, themeMode, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: [
              IconButton(
                icon: Icon(Icons.refresh_rounded, color: isDark ? Colors.white : const Color(0xFF1E1B4B)),
                onPressed: _loadPostDetails,
              ),
              IconButton(
                icon: Icon(Icons.edit_rounded, color: isDark ? Colors.white : const Color(0xFF1E1B4B)),
                onPressed: () {
                  if (_opp != null) {
          ...
          body: StreamBuilder<OpportunityModel?>(
            stream: _firestoreService.streamOpportunityById(widget.opportunityId),
            builder: (context, snapshot) {
              final opp = snapshot.data ?? _opp;

              if (snapshot.connectionState == ConnectionState.waiting && opp == null) {
                return const Center(child: CircularProgressIndicator(color: Color(0xFF3730A3)));
              }
              ...
              final postedDate = DateFormat('MMM dd, yyyy').format(opp.createdAt);
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
                          decoration: BoxDecoration(
                            color: opp.isActive ? const Color(0xFFD1FAE5) : const Color(0xFFF3F4F6),
                            borderRadius: BorderRadius.circular(16),
                          ),
                          child: Text(
                            ...
                            style: TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.bold,
                              color: opp.isActive ? const Color(0xFF047857) : Colors.grey,
                            ),
                          ),
                        ),
                      ],
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          _buildMetricItem(Icons.people_outline_rounded, '${opp.applicantsCount}', 'Applicants', textColor, subtextColor),
                          _buildMetricItem(Icons.calendar_today_rounded, postedDate, 'Posted Date', textColor, subtextColor),
                        ],
                      ),
                    ),
                    const SizedBox(height: 20),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildDetailRow(Icons.work_outline_rounded, 'Work Type', opp.workType, textColor, subtextColor),
                          const SizedBox(height: 14),
                          _buildDetailRow(Icons.access_time_rounded, 'Commitment', opp.commitment, textColor, subtextColor),
                          const SizedBox(height: 14),
                          _buildDetailRow(Icons.location_on_outlined, 'Location', opp.location, textColor, subtextColor),
                          const SizedBox(height: 14),
                          _buildDetailRow(Icons.timer_outlined, 'Hours/Week', opp.hoursPerWeek, textColor, subtextColor),
                        ],
                      ),
                    ),
                    ...
                    Text(opp.description, style: TextStyle(fontSize: 14, color: subtextColor, height: 1.5)),
                    const SizedBox(height: 24),

                    // Skills Required
                    if (opp.skills.isNotEmpty) ...[
                      Text('Required Skills', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: textColor)),
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            icon: Icon(opp.isActive ? Icons.lock_outline_rounded : Icons.lock_open_rounded),
                            label: Text(opp.isActive ? 'Close Post' : 'Reopen Post'),
                            onPressed: () => _toggleActiveStatus(opp),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: opp.isActive ? const Color(0xFFF59E0B) : const Color(0xFF10B981),
                              foregroundColor: Colors.white,
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            label: const Text('Delete Post', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
  ...
  Widget _buildMetricItem(IconData icon, String value, String label, Color textColor, Color subtextColor) {
    return Column(
      children: [
