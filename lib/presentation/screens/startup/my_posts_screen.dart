...
class MyPostsScreen extends StatefulWidget {
  ...
  const MyPostsScreen({super.key, this.onNavTap});
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              nextState
                  ? 'Opportunity reopened! Applicants notified.'
                  : 'Opportunity closed! Applicants notified.',
            ),
            backgroundColor: btnColor,
          ),
        );
            ...
            return Scaffold(
              ...
              body: currentUid == null
                  ...
                  : StreamBuilder<List<OpportunityModel>>(
                      stream: _firestoreService.streamFounderOpportunities(
                        currentUid,
                      ),
                                  ...
                                  SingleChildScrollView(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        _buildFilterTabChip(
                                          'Open',
                                          'Open (${allPosts.where((p) => p.isActive).length})',
                                        ),
                                        const SizedBox(width: 8),
                                        _buildFilterTabChip(
                                          'Closed',
                                          'Closed (${allPosts.where((p) => !p.isActive).length})',
                                        ),
                                      ],
                                    ),
                                  ),
