...
class _ApplicantDetailScreenState extends State<ApplicantDetailScreen> {
  ...
  Future<void> _loadDetails() async {
    try {
      final user = await _firestoreService.getUserProfile(
        widget.application.applicantUid,
      );
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Status updated to "$newStatus". Notification sent to candidate!',
            ),
                          ...
                          : () async {
                              ...
                              if (rootContext.mounted) {
                                ScaffoldMessenger.of(rootContext).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      'Feedback sent successfully to $studentName! Opening Sent Messages...',
                                    ),
  ...
  Widget build(BuildContext context) {
    ...
    final appliedDate = DateFormat(
      'MMM dd, yyyy',
    ).format(widget.application.appliedAt);
    ...
    final countryStr = (user?.country ?? '').isNotEmpty
        ? user!.country!
        : 'Rwanda';
    final fieldOfStudy = (user?.fieldOfStudy ?? '').isNotEmpty
        ? user!.fieldOfStudy!
        : 'Computer Science';
    final educationLevel = (user?.educationLevel ?? '').isNotEmpty
        ? user!.educationLevel!
        : 'Undergraduate';
    ...
    final isEligible =
        eligibleCountries.any(
          (c) => c.toLowerCase() == countryStr.toLowerCase(),
        ) ||
        eligibleCountries.isEmpty;
    ...
    final shortCoverText = coverText.length > 120
        ? '${coverText.substring(0, 120)}...'
        : coverText;
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  isEligible
                                      ? 'Meets Country Eligibility'
                                      : 'Review Country Requirement',
                                  style: TextStyle(
                                    fontSize: 14,
                                    fontWeight: FontWeight.bold,
                                    color: isEligible
